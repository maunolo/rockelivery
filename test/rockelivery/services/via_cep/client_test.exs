defmodule Rockelivery.ViaCep.ClientTest do
  ...
  describe "get_cep_info/1" do
    ...
    test "when there is a invalid cep, returns an error", %{bypass: bypass} do
      ...
      expected_response = {:error, %Error{result: "invalid zip_code", status: :bad_request}}

      assert response == expected_response
    end

    test "when there is a not found cep, returns an error", %{bypass: bypass} do
      cep = "00000000"
      ...
      expected_response = {:error, %Error{result: "zip_code not found", status: :not_found}}
